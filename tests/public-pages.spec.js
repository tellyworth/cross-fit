...
test.describe('WordPress Public Pages', { tag: '@public' }, () => {
  ...
  // Data provider pattern: Test each post type item as a separate step
  // Playwright will run these steps with better isolation and reporting
  test.describe('post type items', () => {
    test('should load post type items without errors', async ({ page, wpInstance }) => {
      ...
      // Test each item as a separate step for better reporting
      for (const item of items) {
        if (item.link) {
          const url = new URL(item.link);
          const path = url.pathname + url.search;
          await test.step(`${item.postTypeName} (${item.postType}): ${path}`, async () => {
            try {
  ...
  // Data provider pattern: Test each list page as a separate step
  // Playwright will run these steps with better isolation and reporting
  test.describe('list pages (archives, search, etc.)', () => {
    test('should load list pages without errors', async ({ page, wpInstance }) => {
      test.setTimeout(60000); // 60 seconds for multiple page loads
      ...
      // Test each list page as a separate step for better reporting
      for (const pageDef of pagesToTest) {
        await test.step(pageDef.description, async () => {
          try {
